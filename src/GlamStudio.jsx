...
function KanbanCard({ entry, config, onAction, isDisabled, isServingDisabled }) {
  ...
  const isNowServingStatus = config.key === "Now Serving_Glam";
...
// ─── Main Component ────────────────────────────────────────────────────────────
export default function GlamStudio({ newEntry, onBack, onLogout, user, onTogaSubmit, onOJTSubmit, onProfileSubmit }) {
  ...
  const handleAction = async (entry, actionType = null) => {
    ...
    if (actionType === "callback") {
      await updateQueueEntryStatus(priorityId, "Entered_Glam");
      await logActivity(user?.id, user?.username, "Callback", "Glam", `${entry.studentName || entry.student_name} (${priorityId}) called back to Entered`);
      refreshQueue();
      return;
    }
    ...
    if (entry.status === "Registered") {
      nextStatus = "Arrived_Glam";
    } else if (entry.status === "Arrived_Glam") {
      nextStatus = "Entered_Glam";
    } else if (entry.status === "Entered_Glam") {
      nextStatus = "Now Serving_Glam";
    } else if (entry.status === "Now Serving_Glam") {
      setRemarksTarget(entry);
      return;
    }
    
    // Show confirmation for Registered→Arrived and Arrived→Entered transitions
    if ((entry.status === "Registered" && nextStatus === "Arrived_Glam") ||
        (entry.status === "Arrived_Glam" && nextStatus === "Entered_Glam")) {
      setConfirmationTarget(entry);
    ...
    if (nextStatus) {
      const actionLabel = nextStatus === "Arrived_Glam" ? "Arrived" :
                         nextStatus === "Entered_Glam" ? "Enter Room" :
                         nextStatus === "Now Serving_Glam" ? "Serving" : "Update";
      await updateQueueEntryStatus(priorityId, nextStatus);
      await logActivity(user?.id, user?.username, "Status Update", "Glam", `${entry.studentName || entry.student_name} (${priorityId}): ${actionLabel}`);
      refreshQueue();
    }
  };
            ...
            <div className="flex flex-col gap-6" style={{ width: "100%" }}>
              ...
              {newEntry && (
                <div className="p-4 sm:p-5 bg-gradient-to-r from-[#1a2f6e]/30 to-[#c9a84c]/20 border border-[#c9a84c]/30 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-5">
                  ...
                  <div>
                    <p className="text-[#e2c06a] text-xs font-semibold uppercase tracking-widest">🎉 Just Registered</p>
